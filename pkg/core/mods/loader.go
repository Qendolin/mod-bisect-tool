...
func populateProviderMaps(allMods map[string]*Mod, potentialProviders PotentialProvidersMap) {
	for _, mod := range allMods {
		...
		providerInfoBase := ProviderInfo{
			TopLevelModID:      mod.Metadata.ID,
			TopLevelModVersion: mod.Metadata.Version.Version,
			ProviderJarPath:    mod.BaseFilename + ".jar",
		}
		...
		for _, nested := range mod.NestedModules {
			...
			nestedProviderInfo.ProviderJarPath = nested.PathInJar
...
func sortAndLogProviders(allMods map[string]*Mod, potentialProviders PotentialProvidersMap) {
	...
	for _, depID := range sortedDepIDs {
		...
		providerLogMessages = append(providerLogMessages, formatProviderLog(depID, infos)...)
	}
...
func formatProviderLog(depID string, infos []ProviderInfo) []string {
	var messages []string
	if len(infos) == 1 {
		info := infos[0]
		messages = append(messages, fmt.Sprintf("  - Dependency %s provided by %s (at v%s) from '%s'",
			depID, info.TopLevelModID, info.VersionOfProvidedItem, info.ProviderJarPath))
	} else {
		...
		for _, info := range infos {
			messages = append(messages, fmt.Sprintf("      - %s (at v%s) from '%s'",
				info.TopLevelModID, info.VersionOfProvidedItem, info.ProviderJarPath))
		}
	}
	return messages
}
...
func sortProviders(infos []ProviderInfo) {
	...
	sort.Slice(infos, func(i, j int) bool {
		...
		if infos[i].TopLevelModVersion.Compare(infos[j].TopLevelModVersion) != 0 {
			return infos[i].TopLevelModVersion.Compare(infos[j].TopLevelModVersion) > 0
		}
		return infos[i].ProviderJarPath < infos[j].ProviderJarPath
	})
}
